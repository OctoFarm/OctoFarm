import {updateProfileDrop, updatePrinterDrops, reRenderPageInformation} from "./filament-manager-ui.utils";
...
export const cloneSpool = async (e) => {
    ...
    const spoolPrinterAssignment = document.getElementById(`spoolsPrinterAssignment-${spoolClonedId}-${nextSpoolIndex}`);
    spoolPrinterAssignment.selectedIndex = 0;

    document.getElementById(`clonedSave-${spoolClonedId}-${nextSpoolIndex}`).addEventListener("click", async function () {
    ...
    document.getElementById(`clonedDelete-${spoolClonedId}-${nextSpoolIndex}`).addEventListener("click", function () {
        const clonedRow = document.getElementById(`clonedRow-${spoolClonedId}-${nextSpoolIndex}`);
        clonedRow.remove();
    });
