...
export class FileStatisticsService {
  ...
  statistics(farmPrinters) {
    ...
    if (fileSizes.length !== 0) {
      ...
      this.fileStats.averageFile = fileSizes.reduce((a, b) => a + b, 0) / fileCount.length;
    }
    if (fileLengths.length !== 0) {
      ...
      this.fileStats.averageLength = fileLengths.reduce((a, b) => a + b, 0) / fileCount.length;
    }
  }
  ...
  generate(farmPrinter, selectedFilament) {
    ...
    if (Number.isNaN(sortIndex) || isNaN(sortIndex)) {
      this.logger.error(`File Cleaner failed: farmPrinter:sortIndex is NaN (${sortIndex})`);
      return;
    }

    // null, string caught here
    if (!sortIndex && !Number.isInteger(sortIndex)) {
      this.logger.error(`File Cleaner failed: farmPrinter:sortIndex not defined (${sortIndex})`);
      return;
    }
    ...
    if (!!fileList?.files) {
      for (let file of fileList.files) {
        ...
        sortedFile.toolUnits = FileStatisticsService.getUnits(selectedFilament, file.length);
