...
setInterval(function() {
    Client.get("client/dash/get").then(res => {
        return res.json();
    }).then(res => {
        ...
        console.log(err)
        UI.createAlert("error", "There was trouble updating page, please check logs", 4000, "clicked")
    });
}, 4000);
...
class dashUpdate {
    static systemInformation(systemInfo){
        ...
        document.getElementById("systemUpdate").innerHTML = Calc.generateTime(systemInfo.sysUptime.uptime);
        ...
        document.getElementById("remainingCPUProg").style.width = 100 - remain + "%";
        let otherRAM = Calc.bytes(systemInfo.memoryInfo.total - systemInfo.memoryInfo.free);
        let octoRAM = Calc.bytes(systemInfo.memoryInfo.total / 100 * systemInfo.sysProcess.pmem);
        let freeRAM = Calc.bytes(systemInfo.memoryInfo.free);
        let otherPer = Math.round((systemInfo.memoryInfo.used / systemInfo.memoryInfo.total * 100) *10) /10;
        let octoPer = Math.round(systemInfo.sysProcess.pmem *10) /10;
        let freePer = 100 - Math.round((systemInfo.memoryInfo.used / systemInfo.memoryInfo.total * 100) *10) /10;
    ...
    static printers(printers){
        printers.forEach(printer => {
            document.getElementById("printerBadge-"+printer.index).innerHTML = printer.current.state;
            document.getElementById("printerBadge-"+printer.index).className = `badge badge-${printer.stateColour.name} badge-pill`;
            if(printer.current.state != "Offline"){
                document.getElementById("printerName-"+printer.index).innerHTML = "<i class='fas fa-print'></i> " + printer.index + ". " + printer.settingsApperance.name;
            }else{
                document.getElementById("printerName-"+printer.index).innerHTML = "<i class='fas fa-print'></i> " + printer.index + ". " + printer.action;
            }
        })
    }
    static currentOperations(farmInfo){
        document.getElementById("completeCount").innerHTML = "Complete: " + farmInfo.complete;
        document.getElementById("idleCount").innerHTML = "Idle: " + farmInfo.idle;
        document.getElementById("activeCount").innerHTML = "Active: " + farmInfo.active;
        document.getElementById("offlineCount").innerHTML = "Offline: " + farmInfo.offline;
        ...
        farmInfo.currentOperations.forEach((current, index) => {
            ...
            if(document.getElementById("currentOpCard-"+current.index)){
                let progress = document.getElementById("currentProgress-"+current.index);
