...
export default class PrinterManager{
    static async init(printers){
        ...
        if(currentIndex === parseInt(document.getElementById("printerIndex").innerHTML)){
        ...
        <div class="col-12 col-sm-12 col-md-4 col-lg-4 col-xl-6">
            ...
            <div class="row">
                <div class="col-12">
                  <img id="printerControlCamera" width="100%" src="http://${printer.camURL}"/>
                </div>
            </div>
        </div>
...
<hr>
        ...
        <div class="row">
          ...
          <div class="col-md-7">
            <div class="row">
              ...
              <div class="col-12 list-group" id="fileLocations"><div class="input-group mb-3"> <div class="input-group-prepend"> <label class="input-group-text bg-secondary text-light" for="fileManagerFolderSelect">Folders:</label> </div> <select class="custom-select bg-secondary text-light" id="fileManagerFolderSelect"><option value="local">local</option></select></div></div>
    <ul class="col-12 list-group border-secondary" id="fileManagerFileList" style="height:200px; overflow-y:scroll;">
      <li id="file-Anubis_Lewis_Final.gcode" class="list-group-item list-group-item-action flex-column align-items-start bg-secondary mb-1">
      <div class="d-flex w-100 justify-content-between">
        <h5 class="mb-1">Anubis Lewis Final.gcode</h5>
        <small><i class="fas fa-stopwatch"></i> 1 Days, 8 Hrs, 55 Mins, 2 Seconds</small>
      </div>
      <small class="float-right"><button id="load-Anubis_Lewis_Final.gcode" role="button" class="btn btn-primary btn-sm"><i class="fas fa-file-upload"></i></button>&nbsp;<button id="select-Anubis_Lewis_Final.gcode" role="button" class="btn btn-success btn-sm"><i class="fas fa-print"></i></button>&nbsp;<button id="delete-Anubis_Lewis_Final.gcode" role="button" class="btn btn-danger btn-sm"><i class="fa fa-trash" aria-hidden="true"></i></button><p></p></small>
    </li>

    
      <li id="file-EVA_T-Shirt_Test.gcode" class="list-group-item list-group-item-action flex-column align-items-start bg-secondary mb-1">
      <div class="d-flex w-100 justify-content-between">
        <h5 class="mb-1">EVA T-Shirt Test.gcode</h5>
        <small><i class="fas fa-stopwatch"></i>  1 Hrs, 4 Mins, 16 Seconds</small>
      </div>
      <small class="float-right"><button id="load-EVA_T-Shirt_Test.gcode" role="button" class="btn btn-primary btn-sm"><i class="fas fa-file-upload"></i></button>&nbsp;<button id="select-EVA_T-Shirt_Test.gcode" role="button" class="btn btn-success btn-sm"><i class="fas fa-print"></i></button>&nbsp;<button id="delete-EVA_T-Shirt_Test.gcode" role="button" class="btn btn-danger btn-sm"><i class="fa fa-trash" aria-hidden="true"></i></button><p></p></small>
    </li>
  ...
  <small class="float-right"><button id="load-EVA_T-Shirt_Test.gcode" role="button" class="btn btn-primary btn-sm"><i class="fas fa-file-upload"></i></button>&nbsp;<button id="select-EVA_T-Shirt_Test.gcode" role="button" class="btn btn-success btn-sm"><i class="fas fa-print"></i></button>&nbsp;<button id="delete-EVA_T-Shirt_Test.gcode" role="button" class="btn btn-danger btn-sm"><i class="fa fa-trash" aria-hidden="true"></i></button><p></p></small>
          ...
          <div class="col-md-5">
            ...
            <div id="pmProgress" class="progress-bar" role="progressbar progress-bar-striped" style="width:${progress.completion}%;" aria-valuenow="${progress.completion}%" aria-valuemin="0" aria-valuemax="100">${progress.completion}%</div>
          </div>
            <b>Expected Print Time: </b><p class="mb-1" id="pmExpectedTime">${Calc.generateTime(job.estimatedPrintTime)}</p>
            <b>Print Time Remaining: </b><p class="mb-1" id="pmTimeRemain">${Calc.generateTime(progress.printTimeLeft)}</p>
            <b>Print Time Elapsed: </b><p class="mb-1" id="pmTimeElapsed">${Calc.generateTime(job.printTime)}</p>
            <b>Current Z: </b><p class="mb-1" id="pmCurrentZ">${printer.currentZ}mm</p>
            <b class="mb-1">File Name: </b><p class="mb-1" id="pmFileName">${job.file.name}</p>
          </div>
        </div>
        `;
        ...
        `;
    document.getElementById("printerControls").insertAdjacentHTML('beforeend',`<div class="row">
...
`);
            ...
            document.getElementById("printerRebootBtn").addEventListener("click", e => {
        ...
        printerPort.innerHTML = '<label for="dashboardSerialPort">Port:</label><select id="pmSerialPort" class="form-control inline"></select>';
        printerBaud.innerHTML = '<label for="dashboardBaudrate">Baudrate:</label><select id="pmBaudrate" class="form-control inline"></select>';
        printerProfile.innerHTML = '<label for="dashboardPrinterProfile">Profile:</label><select id="pmProfile" class="form-control inline"></select>';
        availableBaud.forEach(baud => {
            document.getElementById("pmBaudrate").insertAdjacentHTML('beforeend',`<option value="${baud}">${baud}</option>`);
        })
        if(preferedBaud != null){
            document.getElementById("pmBaudrate").value = preferedBaud;
        }
        availablePort.forEach(port => {
            document.getElementById("pmSerialPort").insertAdjacentHTML('beforeend',`<option value="${port}">${port}</option>`);
        })
        if(preferedPort != null){
            document.getElementById("pmSerialPort").value = preferedPort;
        }
        availableProfile.forEach(profile => {
            document.getElementById("pmProfile").insertAdjacentHTML('beforeend',`<option value="${profile.id}">${profile.name}</option>`);
        })
        if(preferedProfile != null){
            document.getElementById("pmProfile").value = preferedProfile;
        }
        if(printer.state === "Closed"){
            printerConnect.innerHTML = '<center> <button id="pmConnect" class="btn btn-success inline" value="connect">Connect</button></center>';
        }else{
            printerConnect.innerHTML = '<center> <button id="pmConnect" class="btn btn-danger inline" value="disconnect">Disconnect</button></center>';
        }
    ...
    static applyListeners(printer, elements){
        if(printer.state != "Closed"){
            elements.connectPage.connectButton.addEventListener("click", e => {
                ...
                OctoPrintClient.connect(elements.connectPage.connectButton.value, printer);
              })
        }else{
            elements.connectPage.connectButton.addEventListener("click", e => {
                elements.connectPage.connectButton.disabled = true;
    ...
    static grabPage(){
        let printerManager = {
            ...
            jobStatus: {
                ...
                fileManagerFolderSelect: document.getElementById("ileLocations"),
                fileManagerFileList: document.getElementById("fileManagerFolderSelect"),
            }, 
            connectPage: {
    ...
    static async applyState(printer,  job, progress){
        ...
        elements.mainPage.title.innerHTML = "Printer Manager: " + printer.settingsAppearance.name;
        elements.mainPage.status.innerHTML = printer.state;
        elements.mainPage.status.className = `btn btn-${printer.stateColour.name} mb-2`;

        elements.jobStatus.progressBar.innerHTML = progress.completion + "%";
        elements.jobStatus.progressBar.style.width = progress.completion + "%"
        elements.jobStatus.expectedTime.innerHTML = Calc.generateTime(job.estimatedPrintTime);
        elements.jobStatus.remainingTime.innerHTML = Calc.generateTime(progress.printTimeLeft);
        elements.jobStatus.elapsedTime.innerHTML = Calc.generateTime(job.printTime);
        ...
        if(printer.stateColour.category === "Active"){
            elements.printerControls.e0Target.placeholder = printer.temps[0].tool0.target + "°C";
            elements.printerControls.e0Actual.innerHTML = "Actual: " + printer.temps[0].tool0.actual + "°C";
            elements.printerControls.bedTarget.placeholder = printer.temps[0].bed.target + "°C";
            elements.printerControls.bedActual.innerHTML = "Actual: " + printer.temps[0].bed.actual + "°C";
            if(printer.temps[0].tool0.actual > printer.temps[0].tool0.target - 0.5 && printer.temps[0].tool0.actual < printer.temps[0].tool0.target + 0.5){
                elements.printerControls.e0Actual.classList = "input-group-text Complete";
            ...
            if(printer.temps[0].bed.actual > printer.temps[0].bed.target - 0.5 && printer.temps[0].bed.actual < printer.temps[0].bed.target + 0.5){
                elements.printerControls.bedActual.classList = "input-group-text Complete";
            }else if(printer.temps[0].bed.actual < 35){
                elements.printerControls.bedActual.classList = "input-group-text";
            }else{
                elements.printerControls.bedActual.classList = "input-group-text Active";
            }
        ...
        }else if(printer.stateColour.category === "Idle" || printer.stateColour.category === "Complete"){
            ...
            if(typeof printer.temps != 'undefined' && typeof printer.temps[0].tool0 != 'undefined' && typeof printer.temps[0].tool0.target != 'undefined'){
                elements.printerControls.e0Target.placeholder = printer.temps[0].tool0.target + "°C";
                elements.printerControls.e0Actual.innerHTML = "Actual: " + printer.temps[0].tool0.actual + "°C";
                elements.printerControls.bedTarget.placeholder = printer.temps[0].bed.target + "°C";
                elements.printerControls.bedActual.innerHTML = "Actual: " + printer.temps[0].bed.actual + "°C";
        ...
        }else if(printer.stateColour.category === "Offline" || printer.stateColour.category === "Closed"){
