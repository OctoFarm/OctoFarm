...
export default class OctoPrintClient extends Axios {
  ...
  static printerPreHeatChamber(printer, chamberTemp) {
    let chamberData = {
      command: "target",
      target: 0
    };
  ...
  static octoPrintPluginInstallAction(printer, pluginList, action) {
  ...
  static updateOctoPrintPlugins(pluginList, printer) {
    const data = {
      targets: pluginList,
      force: true
    };
  ...
  static updateOctoPrintClient(printer) {
    const data = {
      targets: ["octoprint"],
      force: true
    };
  ...
  static quickConnectPrinterToOctoPrint(printer) {
    let data = {};
  ...
  static disconnectPrinterFromOctoPrint(printer) {
    let data = {
      command: "disconnect"
    };
  ...
  static sendPowerCommandToOctoPrint(printer, powerCommand) {
    if (printer.printerState.colour.category !== "Active") {
  ...
  static printerSendGcode(printer) {
