...
class FileClean {
  ...
  static async statistics(farmPrinters) {
    ...
    fileStatistics.biggestFile = Math.max.apply(
      Math,
      fileCount.map((o) => o.size)
    );
    fileStatistics.smallestFile = Math.min.apply(
      Math,
      fileCount.map((o) => o.size)
    );
    fileStatistics.biggestLength = Math.max.apply(
      Math,
      fileCount.map((o) => o.length)
    );
    fileStatistics.smallestLength = Math.min.apply(
      Math,
      fileCount.map((o) => o.length)
    );
