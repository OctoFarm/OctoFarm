        ...
        if(lengths !== null){
            lengths.forEach((length,index) => {
                ...
                if(typeof filamentSelection !== 'undefined' && Array.isArray(filamentSelection)){
                    if(filamentSelection[index] === null){
                        let radius = 1.75 / 2
                        let volume = (length * Math.PI * radius * radius)
                        let usage = volume * 1.24
                        lengthArray.push(length);
                        weightArray.push(usage);
                        strings.push(`<b>Tool ${index}:</b> ${length.toFixed(2)}m / ${usage.toFixed(2)}g`)
                    }else{
                        if(typeof filamentSelection[index] !== 'undefined'){
                            let radius = parseFloat(filamentSelection[index].spools.profile.diameter) / 2
                            let volume = (length * Math.PI * radius * radius)
                            let usage = volume * parseFloat(filamentSelection[index].spools.profile.density)
                            lengthArray.push(length);
                            weightArray.push(usage);
                            strings.push(`<b>Tool ${index}:</b> ${length.toFixed(2)}m / ${usage.toFixed(2)}g`)
                        }else{
                            lengthArray.push(0);
                            weightArray.push(0);
                            strings.push(`<b>Tool ${index}:</b> (No Spool)`)
                        }
                    }
                }else{
                    lengthArray.push(0);
