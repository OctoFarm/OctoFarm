...
class PrinterClean {
    ...
    static async statisticsStart () {
      ...
      history.currentFailed + activeHours + idleHours + offlineHours;
        ...
        const gasResistanceArray = [];
        //Find min / max values for gas_resistance to tweak calulation...
        const totalRoomData = await RoomData.find({});
        totalRoomData.forEach(data => {
            gasResistanceArray.push(data.gas_resistance);
        });
        const minResistance = Math.min(...gasResistanceArray);
        const maxResistance = Math.max(...gasResistanceArray);
        console.log(minResistance, maxResistance);
        RoomData.find({}).sort('-date')
            .limit(1500).exec(function(err, posts){
                ...
                for(let i = 0; i < enviromentalData.length; i++){
                    currentEnviromentalData[0].data.push({
                        ...
                        y: enviromentalData[i].temperature
                    });
                    currentEnviromentalData[1].data.push({
                        x: enviromentalData[i].date,
                        y: enviromentalData[i].humidity
                    });
                    const iaq = new IAQ(enviromentalData[i].gas_resistance, enviromentalData[i].humidity, 40, minResistance, maxResistance);
                    const values = JSON.parse(iaq.toString());
