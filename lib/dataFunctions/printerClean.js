...
class PrinterClean {
  ...
  static async generate(farmPrinter) {
    try {
      const sortedPrinter = {
      ...
      if (
        typeof sortedPrinter.currentProfile !== "undefined" &&
        sortedPrinter.currentProfile !== null
      ) {
        if (typeof farmPrinter.systemChecks !== "undefined") {
          // Fix for offline printers leaving profile green in checking
          if (farmPrinter.stateColour.category === "Offline") {
            farmPrinter.systemChecks.profile = false;
          } else {
            farmPrinter.systemChecks.profile = true;
          }
        }
      }
  ...
  static async initFarmInformation() {
    ...
    if (typeof farmStats === undefined || farmStats.length < 1) {
      ...
      console.log(newfarmStats);
      farmStats[0] = newfarmStats;
      console.log(farmStats);
      newfarmStats.save();
