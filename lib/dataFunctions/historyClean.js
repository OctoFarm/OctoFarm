...
class HistoryClean {
    ...
    static async start() {
        ...
        for (let h = 0; h < history.length; h++) {
            ...
            sorted.costPerHour = parseFloat(sorted.totalCost) / parseFloat(((history[h].printHistory.printTime / 360000)) * 100);
            sorted.printHours = await HistoryClean.getHours(history[h].printHistory.printTime);
    ...
    static async getJob(job, printTime) {
        if (typeof job !== "undefined") {
            const accuracy = ((printTime - job.estimatedPrintTime) / printTime) * 10000;
            const newJob = {
