            ...
            if(typeof sorted.spools !== 'undefined' && sorted.spools !== null){
                ...
                for(let s=0; s< sorted.spools.length; s++){
                    if(typeof sorted.spools[s]["tool"+keys[s]] !== 'undefined'){
                        spoolCost = spoolCost + numOr0(sorted.spools[s]["tool"+keys[s]].cost)
                        totalVolume = totalVolume + numOr0(sorted.spools[s]["tool"+keys[s]].volume)
                        totalLength = totalLength + numOr0(sorted.spools[s]["tool"+keys[s]].length)
                        totalWeight = totalWeight + numOr0(sorted.spools[s]["tool"+keys[s]].weight)
                    }
                }
            }
