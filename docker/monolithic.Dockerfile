...
RUN wget -qO - https://www.mongodb.org/static/pgp/server-4.4.1.asc | apt-key add
RUN echo 'deb http://repo.mongodb.org/apt/debian stretch/mongodb-org/4.4.1 main' | tee /etc/apt/sources.list.d/mongodb.list
RUN apt-get update
